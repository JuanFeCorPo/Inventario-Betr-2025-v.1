    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Dar de Baja Equipo">
            <div className="space-y-4">
                <p>Especifica el motivo para dar de baja este equipo.</p>
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Ej: Pantalla rota, obsoleto..." className="w-full bg-gray-700 p-3 rounded-lg h-28" required />
                <div className="flex justify-end space-x-4 pt-2">
                    <button type="button" onClick={onClose} className="px-6 py-2 rounded-lg bg-gray-600 hover:bg-gray-500">Cancelar</button>
                    <button onClick={handleConfirm} className="px-6 py-2 rounded-lg bg-yellow-600 hover:bg-yellow-500 font-semibold">Confirmar Baja</button>
                </div>
            </div>
        </Modal>
    );
...
const HistoryModal = ({ isOpen, onClose, item }) => (
    <Modal isOpen={isOpen} onClose={onClose} title={`Historial de ${item?.nombre}`}>
        <div className="space-y-4 text-gray-300">
            <p><span className="font-semibold text-gray-100">Obs. Generales:</span> {item?.observaciones || 'Ninguna.'}</p>
            {item?.estado === 'De Baja' && (
                <div className="bg-yellow-900/50 p-4 rounded-lg border border-yellow-700">
                    <h4 className="font-bold text-yellow-300">Info de Baja</h4>
                    <p><span className="font-semibold">Fecha:</span> {item.fecha_baja?.toDate().toLocaleString()}</p>
                    <p><span className="font-semibold">Motivo:</span> {item.motivo_baja || 'No especificado.'}</p>
                </div>
            )}
            <p className="italic text-sm text-center pt-4">Historial de asignaciones futuro.</p>
        </div>
    </Modal>
);
...
const ItemFormModal = ({ isOpen, onClose, onSave, currentItem }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setItem(currentItem || { nombre: '', categoria: categorias[0], estado: 'Disponible' });
        }
    }, [isOpen, currentItem]);
    ...
    const handleSave = (e) => {
        ...
        if (item.fechaIngreso && typeof item.fechaIngreso === 'string') {
            dataToSave.fechaIngreso = Timestamp.fromDate(new Date(item.fechaIngreso));
        }
        onSave(dataToSave);
    };
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={currentItem ? 'Modificar Equipo' : 'Añadir Nuevo Equipo'}>
            <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input name="nombre" value={item.nombre || ''} onChange={handleChange} placeholder="Nombre del Equipo" className="bg-gray-700 p-3 rounded-lg" required />
                    <select name="categoria" value={item.categoria || ''} onChange={handleChange} className="bg-gray-700 p-3 rounded-lg">{categorias.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select>
                    <input name="numeroSerial" value={item.numeroSerial || ''} onChange={handleChange} placeholder="Número de Serial" className="bg-gray-700 p-3 rounded-lg" />
                    <input name="numeroInventario" value={item.numeroInventario || ''} onChange={handleChange} placeholder="Número de Inventario" className="bg-gray-700 p-3 rounded-lg" required />
                    <input type="date" name="fechaIngreso" value={item.fechaIngreso ? (typeof item.fechaIngreso === 'string' ? item.fechaIngreso : new Date(item.fechaIngreso.seconds * 1000).toISOString().split('T')[0]) : ''} onChange={handleChange} className="bg-gray-700 p-3 rounded-lg" required />
                    <select name="estado" value={item.estado || 'Disponible'} onChange={handleChange} className="bg-gray-700 p-3 rounded-lg"><option>Disponible</option><option>En Uso</option><option>En Mantenimiento</option></select>
                    <textarea name="observaciones" value={item.observaciones || ''} onChange={handleChange} placeholder="Observaciones" className="bg-gray-700 p-3 rounded-lg md:col-span-2 h-24" />
                </div>
                <div className="flex justify-end space-x-4 pt-4">
                    <button type="button" onClick={onClose} className="px-6 py-2 rounded-lg bg-gray-600 hover:bg-gray-500">Cancelar</button>
                    <button type="submit" className="px-6 py-2 rounded-lg bg-orange-600 hover:bg-orange-500 font-semibold">Guardar</button>
                </div>
            </form>
        </Modal>
    );
    ...
    <div className="animate-modal-in">
        <header className="flex justify-between items-center mb-8">
            <div>
                ...
                <p className="text-gray-400">Esta sección es una simulación visual.</p>
            </div>
        </header>
        <div className="bg-gray-800 p-6 rounded-xl text-center text-gray-300">
            <p>La gestión de usuarios real se implementa con Cloud Functions por seguridad.</p>
            <button onClick={onBack} className="mt-8 px-6 py-2 rounded-lg bg-gray-600 hover:bg-gray-500">Volver</button>
        </div>
    </div>
);
...
export default function App() {
    ...
    return <>{user ? <InventoryDashboard user={user} onLogout={handleLogout} db={db} /> : <LoginScreen onLogin={handleLogin} />}</>;
}