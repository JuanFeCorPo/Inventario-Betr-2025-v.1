...
// Un Modal genérico para evitar repetir código
...
const LoginScreen = ({ onLogin }) => {
    // (Sin cambios significativos, funciona como antes)
...
const InventoryDashboard = ({ user, onLogout, db, auth }) => {
    ...
    const [modal, setModal] = useState({ type: null, data: null }); // 'add', 'edit', 'history', 'deactivate'
    ...
    const handleSaveItem = async (itemData) => {
        ...
        try {
            if (modal.data?.id) { // Editando
                const itemRef = doc(db, itemsCollectionPath, modal.data.id);
                await updateDoc(itemRef, itemData);
            } else { // Creando
                await addDoc(collection(db, itemsCollectionPath), { ...itemData, addedBy: user.uid, createdAt: Timestamp.now() });
            }
    ...
    const handleDeactivateItem = async (reason) => {
        ...
        await updateDoc(itemRef, { 
            ...
            motivo_baja: reason // Nuevo campo
        });
        setModal({ type: null, data: null });
    };
    ...
    // ... (El resto de la lógica de filtros y estadísticas, sin cambios mayores)
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Historial de ${item?.nombre}`}>
            <div className="space-y-4 text-gray-300">
                <p><span className="font-semibold text-gray-100">Observaciones Generales:</span> {item?.observaciones || 'Ninguna.'}</p>
                {item?.estado === 'De Baja' && (
                    <div className="bg-yellow-900/50 p-4 rounded-lg border border-yellow-700">
                        <h4 className="font-bold text-yellow-300">Información de Baja</h4>
                 ...
                 {/* Aquí se agregará el historial de asignaciones */}
