...
export default function App() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            initializeAuth().catch(err => {
                console.error("Fallo crítico en la autenticación", err);
                setConfigError(true);
                setLoading(false);
            });
            ...
            console.error("CRITICAL: La inicialización de Firebase falló.", error);
            setConfigError(true);
            setLoading(false);
        }
    }, []);
    ...
    const filteredItems = useMemo(() => items.filter(item => { const catMatch = filterCategory === 'Todos' || item.categoria === filterCategory; const statusMatch = (filterStatus === 'Activos' && item.estado !== 'De Baja') || (filterStatus === 'De Baja' && item.estado === 'De Baja') || filterStatus === 'Todos'; const searchMatch = searchTerm === '' || item.nombre.toLowerCase().includes(searchTerm.toLowerCase()) || (item.numeroSerial && item.numeroSerial.toLowerCase().includes(searchTerm.toLowerCase())) || item.numeroInventario.toLowerCase().includes(searchTerm.toLowerCase()); return catMatch && statusMatch && searchMatch; }), [items, filterCategory, filterStatus, searchTerm]);
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-gray-100 font-sans p-4 sm:p-6 lg:p-8">
                ...
                {view === 'inventory' ? (
                    <div className="animate-modal-in">
                        <header className="flex flex-wrap gap-4 justify-between items-center mb-8">
                            <div><h1 className="text-3xl font-bold text-white">Sistema de Inventario</h1><p className="text-gray-400">Panel de gestión para equipos de marketing.</p></div>
                            <div className="flex items-center gap-4"><button onClick={() => setView('users')} className="flex items-center space-x-2 bg-gray-700 text-white px-5 py-3 rounded-xl font-semibold hover:bg-gray-600 transition-colors"><Users size={20} /><span>Usuarios</span></button><button onClick={openAddItemModal} className="flex items-center space-x-2 bg-orange-600 text-white px-5 py-3 rounded-xl font-semibold hover:bg-orange-500 transition-all duration-300 shadow-lg hover:shadow-orange-500/50"><PlusCircle size={20} /><span>Añadir Equipo</span></button></div>
                        </header>
            ...
            <ConfirmDialog isOpen={showConfirm} onClose={() => setShowConfirm(false)} onConfirm={executeConfirmAction} title={confirmAction.action === 'soft-delete' ? 'Dar de Baja' : 'Eliminar Equipo'} message={confirmAction.action === 'soft-delete' ? `El equipo será marcado como 'De Baja' y se ocultará.` : `¡ADVERTENCIA! Esta acción es irreversible.`}/>
            <style>{`
